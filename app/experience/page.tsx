...
export default function Experience() {
  ...
  return (
    <motion.section
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className='mt-20 scroll-mt-28 mb-28 sm:mb-40'>
      ...
      <motion.a
        href='https://drive.google.com/drive/my-drive?hl=vi'
        target='_blank'
        rel='noopener noreferrer'
        className='text-btn underline italic font-semibold text-lg'>
        Download CV / My Bachelor's Degree
      </motion.a>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
            ...
            experiencesData.map((item: any, index: number) => (
              <React.Fragment key={index}>
                <VerticalTimelineElement
                  ...
                  date={item?.date}
                  icon={
                    index === 0 ? React.createElement(LuGraduationCap)
                    : index === 1 ?
